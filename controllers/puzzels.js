...
const {token}=req.cookies; 
 ...
 if(answerString==="potato"){
  ...
  cont=cont+1;
  if(cont===1){
   res.render('puzzle1.ejs',{title:`Hey,${score.name}`,message:"Oops, Wrong Answers.Only one chance left!",hint:"Hint:Think some root vegetable."});
  }
  if(cont===2){
...
export const puzzle2 =async(req,res)=>{
 ...
 if(answerString==="12 angry men"){
  ...
  cont=cont+1;
  if(cont===1){
   res.render('puzzle2.ejs',{title:`Hey,${score.name}`,message:"Oops, Wrong Answers.Only one chance left!",hint:"Hint:movie release in 1957"});
  }
  if(cont===2){
    res.cookie("token",null,{
      expires:new Date(Date.now()) // in millisecond
...
//  console.log(answerString);
 if(answerString==="tamanna bhatia"){
  ...
  cont=cont+1;
  if(cont===1){
   res.render('puzzle3.ejs',{title:`Hey,${score.name}`,message:"Oops, Wrong Answers.Only one chance left!",hint:"Hint 1: Her name contain *Desire*."});
  }
  if(cont===2){
    res.cookie("token",null,{
      expires:new Date(Date.now()) // in millisecond
...
export const puzzle4 =async(req,res)=>{
 ...
 if(answerString==="russia"){
  ...
  cont=cont+1;
  if(cont===1){
   res.render('puzzle4.ejs',{title:`Hey,${score.name}`,message:"Oops, Wrong Answers.Only one chance left!",hint:"Hint: This country has an area of 17.1 million km2"});
  }
  if(cont===2){
    res.cookie("token",null,{
      expires:new Date(Date.now()) // in millisecond
...
export const puzzle5= async(req,res)=>{
 ...
 if(answerString==="clock"){
  ...
  cont=cont+1;
  if(cont===1){
   res.render('puzzle5.ejs',{title:`Hey,${score.name}`,message:"Oops, Wrong Answers.Only one chance left!",hint:"P.S: I went easy on you because this is your last puzzle. I may not give more hints but first see you wrist."});
  }
  if(cont===2){
    res.cookie("token",null,{
      expires:new Date(Date.now()) // in millisecond
